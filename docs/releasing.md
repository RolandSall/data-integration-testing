...
1. Run `bun run verify`, `bun run pack:check:docker`, `bun run test:runners:minimum`,
   and `bun run test:consumer` with Docker available. Repository CI, including the isolated in-repository example consumer check, must pass.
...
6. Verify the published version in a clean consumer. The in-repository example gate continues
   testing freshly packed output outside the workspace; examples are never included in the npm archive.
