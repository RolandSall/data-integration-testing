...
// These APIs are available in the published 0.1.0 release.
const lifecycle = createVitestContainerGlobalSetup({
  ...
  prepareResources: async (resources) => {
    const pool = new Pool({ connectionString: resources.getNamed('database', Container.PostgreSql).connectionUri });
