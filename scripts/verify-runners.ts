...
export const dataContext = createDataIntegrationTestContext({
  ...
  closeClient: async () => { record('close'); if (scenario === 'cleanup-failure' || scenario === 'setup-failure' || scenario === 'assertion-cleanup-failure') throw new Error('close failure'); },
...
for (const runner of ['jest', 'vitest'] as const) {
  ...
  for (const scenario of cases) {
    ...
    if ('additionalError' in scenario && scenario.additionalError) assert.match(result.output, new RegExp(scenario.additionalError, 'i'), message);
